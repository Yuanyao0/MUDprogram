...
static void readGameData(Player* player, petHouse* pethouse, const std::string& filename) {
    ...
    if (file.is_open()) {
        ...
        for (int i = 0; i < Num; i++) {
            player->getBag()->getPetByIndex(i)->init(player->getBag()->getPetByIndex(i)->getNowLife(),
                player->getBag()->getPetByIndex(i)->getNowPP(), player->getBag()->getPetByIndex(i)->getLevel(), player->getBag()->getPetByIndex(i)->getaseed());
        }
